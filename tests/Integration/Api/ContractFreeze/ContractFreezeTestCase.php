...
namespace App\Helpers {
    ...
    // the function inside App\Helpers lets contract tests opt into determinism through
    // the flag on ContractFreezeTestCase while every other caller falls through to the
    // real generator. This block lives here because the base class is guaranteed to
    // load before any contract test, so there is exactly one definition.
    if (!function_exists(__NAMESPACE__ . '\random_int')) {
        function random_int(int $min, int $max): int
        {
            return ContractFreezeTestCase::$pinRandomInt
                ? $min
                : \random_int($min, $max);
        }
    }
...
namespace App\Tests\Integration\Api\ContractFreeze {
    ...
    /**
     * Base class for the frozen 1.0 API contract suite. Every response captured here is
     ...
     * test class must carry #[Group('contract-freeze')] itself. CI relies on that group
     * to keep this suite off the non-sqlite database jobs.
     */
    abstract class ContractFreezeTestCase extends IntegrationTestCase
    {
        use BuildsContractWorld;
         ...
         */
        protected function assertContractSnapshot(TestResponse $response, int $expectedStatus = 200): void
        {
         ...
         * keep ours next to the test class instead so the frozen contract stays browsable.
         */
