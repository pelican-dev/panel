...
class DatabaseMutationContractTest extends ContractFreezeTestCase
{
    ...
    {
        $this->markTestSkipped('POST /databases currently 500s: DatabaseManagementService::create logs its own activity inside the controller Activity transaction, resetting the pending log, so the wrapper then saves an event-less activity row and violates the NOT NULL constraint. Freeze the contract once that double log is fixed.');
    }
    ...
    {
        $this->assertContractSnapshot($this->postJson(self::BASE . '/databases/100/rotate-password?include=password'));
    }
}
