...
class StoreEndpointsContractTest extends ContractFreezeTestCase
{
    use MocksUuids;
    ...
    public function test_store_user(): void
    {
        $response = $this->postJson('/api/application/users', [
            'email' => 'stored@contract.test',
            'username' => 'contract.stored',
            'password' => 'contract-stored-password',
            'language' => 'en',
            'timezone' => 'UTC',
        ]);

        $this->assertContractSnapshot($response, 201);
    }
    ...
    public function test_store_node(): void
    {
        ...
        $response = $this->postJson('/api/application/nodes', [
            'name' => 'contract-stored-node',
            'fqdn' => 'stored.contract.test',
            'scheme' => 'https',
            'memory' => 2048,
    ...
    public function test_store_database_host(): void
    {
        ...
        $response = $this->postJson('/api/application/database-hosts', [
            'name' => 'contract-stored-host',
            'host' => '192.0.2.30',
            'port' => 3306,
            'username' => 'contract-store',
            'password' => 'contract-stored-host-password',
            'node_ids' => [100],
        ]);
    ...
    public function test_import_egg(): void
    {
        ...
        $yaml = <<<'YAML'
            meta:
              version: PLCN_v3
            name: Contract Imported Egg
            author: import@contract.test
            uuid: 00000000-0000-4000-8000-000000000700
            description: A pinned imported egg for the contract freeze suite.
            docker_images:
              'Java 21': 'ghcr.io/contract/java:21'
            startup_commands:
            ...
            scripts:
              installation:
                script: 'echo install'
                container: 'ghcr.io/contract/installer:latest'
                entrypoint: 'ash'
            variables: []
            YAML;
    ...
    public function test_store_server(): void
    {
        ...
        $response = $this->postJson('/api/application/servers', [
            'external_id' => 'contract-stored-ext',
            'name' => 'Contract Stored Server',
            'description' => 'A pinned stored server for the contract freeze suite.',
            'user' => 101,
            'egg' => 100,
            'docker_image' => 'ghcr.io/contract/java:21',
