            ...
            ->actions([
                ...
                ActionGroup::make([
                    Action::make('rename')
                        ...
                        ->action(function ($data, File $file, DaemonFileRepository $daemonFileRepository) use ($server) {
                    ...
                    Action::make('copy')
                        ...
                        ->action(function (File $file, DaemonFileRepository $daemonFileRepository) use ($server) {
                            $daemonFileRepository
                                ->setServer($server)
                    ...
                    Action::make('download')
                        ...
                        ->visible(fn (File $file) => $file->is_file)
                        ->action(function (File $file, NodeJWTService $nodeJWTService) use ($server) {
                            $token = $nodeJWTService
            ...
            ->bulkActions([
                BulkActionGroup::make([
                    ...
                    BulkAction::make('archive')
                        ->authorize(fn () => auth()->user()->can(Permission::ACTION_FILE_ARCHIVE, $server))
                        ->action(function (Collection $files, DaemonFileRepository $daemonFileRepository) use ($server) {
    ...
    protected function getHeaderActions(): array
    {
        ...
        return [
            HeaderAction::make('new_file')
                ...
                ->action(function ($data, DaemonFileRepository $daemonFileRepository) use ($server) {
                    $daemonFileRepository
                        ->setServer($server)
