...
class ListEggs extends ListRecords
{
    ...
    public function table(Table $table): Table
    {
            ...
            ->columns([
                ...
                ImageColumn::make('image')
                    ...
                    ->tooltip(function ($record) {
                        ...
                        $server = Server::where('egg_id', $record->id)->first();

                        if (!$server) {
                            return 'No Servers Assigned This Egg.';
                        }

                        $serverAddress = $server->allocation?->address ?? 'N/A';
                        $actiongroup = ListServers::getPowerActionGroup()->record($server);
                        $actiongroupHtml = $actiongroup->isVisible()
            ...
            <svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round' class='text-primary'>
               ...
               <div style=\"
                ...
                \"></div>"
                        ...
                        return Html::make("
            <div style='width:680px; font-family:sans-serif; position: relative; cursor: default;'>
                <div style='position: absolute; left:0; top:1px; bottom:0; width:4px; background-color: {$server->condition->getColor(true)}; border-radius:2px;'></div>

                <div style='flex:1; background-color:#1f2937; color:white; border-radius:8px; overflow:hidden; padding:12px; position:relative;'>
                    {$eggImageDiv}

                    <div style='display:flex; align-items:center; margin-bottom:12px; gap:8px;'>
                        <span style='display:inline-block; width:32px; height:32px; background-color: {$server->condition->getColor()}; border-radius:50%;'></span>
                        <h2 style='font-weight:bold; font-size:1.25rem;'>
                            {$server->name} <span style='color:#9ca3af;'>({$server->formatResource(ServerResourceType::Uptime)})</span>
                        </h2>
                        <div class='end-0'>
                    ...
                    <div style='display:flex; justify-content:space-between; gap:16px; text-align:center;'>
                        <div>
                            <p style='font-size:0.75rem; color:#9ca3af;'>CPU</p>
                            <p style='font-weight:600;'>{$server->formatResource(ServerResourceType::CPU)}</p>
                            <hr style='margin:2px 0'>
                            <p style='font-size:0.625rem; color:#9ca3af;'>{$server->formatResource(ServerResourceType::CPULimit)}</p>
                        </div>
                        <div>
                            <p style='font-size:0.75rem; color:#9ca3af;'>Memory</p>
                            <p style='font-weight:600;'>{$server->formatResource(ServerResourceType::Memory)}</p>
                            <hr style='margin:2px 0'>
                            <p style='font-size:0.625rem; color:#9ca3af;'>{$server->formatResource(ServerResourceType::MemoryLimit)}</p>
                        </div>
                        <div>
                            <p style='font-size:0.75rem; color:#9ca3af;'>Disk</p>
                            <p style='font-weight:600;'>{$server->formatResource(ServerResourceType::Disk)}</p>
                            <hr style='margin:2px 0'>
                            <p style='font-size:0.625rem; color:#9ca3af;'>{$server->formatResource(ServerResourceType::DiskLimit)}</p>
                        </div>
                        <div>
                            <p style='font-size:0.75rem; color:#9ca3af;'>Network</p>
                            <hr style='margin:2px 0'>
                            <p style='font-weight:600;'>{$serverAddress}</p>
                        </div>
                    </div>
                </div>
            </div>
        ");
                    ...
                    ->extraAttributes([
                        'x-tooltip-class' => 'max-w-none p-0 w-[680px]',
                    ])
